...
async def globals_middleware_dispatch(
    ...
    """Dispatch the request preserving FastAPI's request scope.
    Avoid copy_context() here to keep fastapi_inner_astack available.
    """
    response = await call_next(request)
    return response


class GlobalsMiddleware:  # ASGI middleware without BaseHTTPMiddleware
    def __init__(self, app: ASGIApp) -> None:
        self.app = app

    async def __call__(self, scope, receive, send) -> None:
        # Pass-through, preserves FastAPI's request scope (fastapi_inner_astack)
